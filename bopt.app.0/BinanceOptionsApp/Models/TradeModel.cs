...
namespace BinanceOptionsApp.Models
{
    ...
    public class OpenOrderSettingsModel : BaseModel
    {
        ...
        private decimal shiftBid;
        public decimal ShiftBid
        {
            get { return shiftBid; }
            set { if (shiftBid != value) { shiftBid = value; OnPropertyChanged(); } }
        }

        private decimal _ShiftAsk;
        public decimal ShiftAsk
        {
            get { return _ShiftAsk; }
            set { if (_ShiftAsk != value) { _ShiftAsk = value; OnPropertyChanged(); } }
        }

        private bool avtoSettings;
        public bool AvtoSettings
        {
            get { return avtoSettings; }
            set { if (avtoSettings != value) { avtoSettings = value; OnPropertyChanged(); } }
        }
    ...
    public class TradeModel : BaseModel
    {
        public TradeModel()
        {
            SPOT = new ProviderModel();
            OPTION = new ProviderModel();
            Slow2 = new ProviderModel();
            Providers = new ObservableCollection<ProviderModel>();
            SPOTProviders = new ObservableCollection<ProviderModel>();
            OPTIONProviders = new ObservableCollection<ProviderModel>();
            SPOT.Parent = this;
            OPTION.Parent = this;
            Slow2.Parent = this;
            Point = 0.00001M;
        ...
        private ProviderModel _SPOT;
        public ProviderModel SPOT
        {
            get { return _SPOT; }
            set { if (_SPOT != value) { _SPOT = value; OnPropertyChanged(); CreateTitle(); } }
        }
        private ProviderModel _OPTION;
        public ProviderModel OPTION
        {
            get { return _OPTION; }
            set { if (_OPTION != value) { _OPTION = value; OnPropertyChanged(); CreateTitle(); } }
        }
        ...
        private ObservableCollection<ProviderModel> _SPOTProviders;
        public ObservableCollection<ProviderModel> SPOTProviders
        {
            get { return _SPOTProviders; }
            set { if (_SPOTProviders != value) { _SPOTProviders = value; OnPropertyChanged(); } }
        }
        private ObservableCollection<ProviderModel> _OPTIONProviders;
        public ObservableCollection<ProviderModel> OPTIONProviders
        {
            get { return _OPTIONProviders; }
            set { if (_OPTIONProviders != value) { _OPTIONProviders = value; OnPropertyChanged(); } }
        }
        ...
        public void CreateTitle()
        {
            if (!Model.Loading)
            {
                ...
                if (Algo == TradeAlgorithm.TwoLegLock) title = App.LanguageKey("locAlgoTitle2LegLock");
                if (Algo == TradeAlgorithm.TwoLegSimpleHedge) title = App.LanguageKey("locAlgoTitle2LegSimple");
                if (Algo == TradeAlgorithm.MultiLegSpread) title = App.LanguageKey("locAlgoTitleMultileg");
                if (Algo == TradeAlgorithm.OneLegHedge) title = "1LEGHEDGE";
                if (Algo == TradeAlgorithm.OneLegMulti) title = "1LEGMULTI";

                if (OPTION != null && Algo != TradeAlgorithm.MultiLegSpread && Algo!= TradeAlgorithm.OneLegMulti)
                {
                    title += " " + OPTION.Symbol;
                    var slowProvider = OPTION != null ? Model.AllConnections.FirstOrDefault(x => x.Name == OPTION.Name) : null;
                    if (slowProvider != null)
                    {
                        title += " " + slowProvider.UIShortName();
                    }
                }
                Title = title.Trim();
            }
