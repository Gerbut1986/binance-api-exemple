...
namespace BinanceOptionsApp.Models
{
    public class ConfigModel : BaseModel
    {
        ...
        public static ConfigModel Load(string filename)
        {
            ...
            {
                if (tab.SPOT == null) tab.SPOT = new ProviderModel();
                if (tab.OPTION == null) tab.OPTION = new ProviderModel();
                if (tab.Slow2 == null) tab.Slow2 = new ProviderModel();
                if (tab.Providers == null) tab.Providers = new ObservableCollection<ProviderModel>();
                if (tab.SPOTProviders == null) tab.SPOTProviders = new ObservableCollection<ProviderModel>();
                if (tab.OPTIONProviders == null) tab.OPTIONProviders = new ObservableCollection<ProviderModel>();
                tab.SPOT.Parent = tab;
                tab.OPTION.Parent = tab;
                tab.Slow2.Parent = tab;
                foreach (var provider in tab.Providers) provider.Parent = tab;
                foreach (var provider in tab.OPTIONProviders) provider.Parent = tab;
                foreach (var provider in tab.SPOTProviders) provider.Parent = tab;
