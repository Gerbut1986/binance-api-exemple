...
namespace BinanceOptionsApp
{
    public partial class TradeMultiLegSpread : UserControl, IConnectorLogger, ITradeTabInterface
    {
        ...
        public void InitializeTab()
        {
            ...
            fast.InitializeProviderControl(model.SPOT,false);
        ...
        private void ThreadProc()
        {
            ...
            model.SPOT.Bid = 0;
            model.SPOT.Ask = 0;
            model.SPOT.Time = DateTime.MinValue;
            model.SPOT.Volume = 0;
            model.SPOT.Balance = null;
            model.OPTION.Bid = 0;
            model.OPTION.Ask = 0;
            model.OPTION.Time = DateTime.MinValue;
            model.OPTION.Volume = 0;
            model.OPTION.Balance = null;
            model.GapBuy = 0;
            model.GapSell = 0;
            ...
            while (!threadStop.WaitOne(model.SleepMs))
            {
                ...
                if (model.FeederOk)
                {
                    ...
                    for (int i=0;i<connectors.Count;i++)
                    {
                        ...
                        if (i == 0)
                        {
                            model.SPOT.Ask = connectors[i].Info.Ask;
                            model.SPOT.Bid = connectors[i].Info.Bid;
                            model.SPOT.Volume = connectors[i].Info.Volume;
                            model.SPOT.Time = connectors[i].Info.Time;
                            model.SPOT.Balance = connectors[i].Info.Balance;
                        }
                        if (i == 1)
                        {
                            model.OPTION.Ask = connectors[i].Info.Ask;
                            model.OPTION.Bid = connectors[i].Info.Bid;
                            model.OPTION.Volume = connectors[i].Info.Volume;
                            model.OPTION.Time = connectors[i].Info.Time;
                            model.OPTION.Balance = connectors[i].Info.Balance;
                        }
                    }
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                Name = model.SPOT.Name,
                Symbol = model.SPOT.Symbol,
                Prefix = model.SPOT.Prefix,
                Postfix = model.SPOT.Postfix,
                Parent = model,
