    ...
    internal class BinanceOptionClient : BaseCryptoClient
    {
        ...
        internal async Task<decimal> GetBalance(string Asset)
        {
            ...
            try
            {
                ...
                {
                    AccountInfo AI = await CallAsync<AccountInfo>(EndPoints.AccountInformationOption, $"recvWindow={recvWindow}");
        ...
        {
            var reqType = "BTC@openInterest@231228/BTC@openInterest@231231";
        ...
        void SetOI(string json)
        {
            ...
            for (int i = 0; i < model.data.Count(); i++)
            {
                ...
                if (found == null)  // Якщо у всьому массиві не має поточного страйку
                    SetStrikeDoesntExist(TYPE, STRIKE, exDate, OIcntrct, OIusd);
                else // Якщо елемент з властивістю 'Strike' існує:
                    SetStrikeExist(found, TYPE, OIcntrct, OIusd);
        ...
        private async void StartUserDataStream()
        {
            try
            {
                BinanceEndPoint ep = (AccountTradeType == AccountTradeType.OPTION)
                    ? EndPoints.SpotCreateListenKeyOption : EndPoints.MarginCreateListenKeyOption;
        ...
        private OptChain SetStrikeDoesntExist(string type, string strike, string exDate, string OIcntrct, string OIusd)
        {
            var m = new OptChain();
            if (type == "C")
            {
        ...
        private void SetStrikeExist(OptChain found, string type, string OIcntrct, string OIusd)
        {
            if (found != null)
            {
