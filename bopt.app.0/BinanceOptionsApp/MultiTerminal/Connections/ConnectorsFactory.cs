using BinanceOptionsApp.MultiTerminal.Connections;
...
namespace MultiTerminal.Connections
{
    ...
    internal class ConnectorsFactory
    {
        ...
        public IConnector CreateBinanceFutures(IConnectorLogger logger, ManualResetEvent cancelToken, BinanceFutureConnectionModel model)
        {
            lock (connectors)
            {
                var connector = CreateExist(model.Name);
