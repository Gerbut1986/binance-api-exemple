...
namespace BinanceOptionsApp.MultiTerminal.Connections
{
    internal class BinanceTestnetCryptoClient : BaseCryptoClient
    {
        ...
        {
            WebSocket wsDepth = new WebSocket($"wss://stream.binancefuture.com/ws/{data.quote.Symbol.ToLower()}@depth");
        ...
        private async void StartUserDataStream()
        {
            try
            {
                BinanceEndPoint ep = (AccountTradeType == AccountTradeType.SPOT) ? EndPoints.SpotCreateListenKeyT : EndPoints.MarginCreateListenKey;
