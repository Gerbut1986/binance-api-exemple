...
namespace BinanceOptionsApp
{
    ...
    internal class TradeCommandOpen : TradeCommand
    {
        ...
        public TradeCommandOpen(OrderSide side, double price, double volume, int slippagePt, EntryOrderType type, int pendingDistancePt, int pendingLifetimeMs)
        {
            CommandSide = side;
            CommandPrice = price;
    ...
    internal class TradeProcessor
    {
        ...
        public void OpenOrder(OrderSide side, double price, double volume, int slippagePt, DateTime currentTime, EntryOrderType orderType, int pendingDistancePt, int pendingLifetimeMs)
        {
            var command = new TradeCommandOpen(side, price, volume, slippagePt, orderType, pendingDistancePt,pendingLifetimeMs);
            if (backtest)
            {
                ...
                if (cmds != null)
                {
                    foreach (var cmd in cmds)
                    {
                        if (cmd is TradeCommandOpen to)
                        {
                            ...
                            var openResult = connector.Open(symbol, (decimal)commandPrice, (decimal)to.CommandVolume, to.CommandSide, magic, to.CommandSlippagePt, 1, commandOrderType, -1);
                            if (openResult.Error==null)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (filledOrder==null)
                                    {
                                        connector.OrderDelete(openResult.Id, symbol, commandOrderType, to.CommandSide, (decimal)to.CommandVolume, (decimal)commandPrice);
                                        filledOrder = DetectOpenedMarketOrder(to, openResult);
                                    }
                                    if (filledOrder!=null)
                                    {
