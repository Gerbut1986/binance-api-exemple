...
namespace BinanceOptionsApp
{
    public partial class TradeMultiLegSpread : UserControl, IConnectorLogger, ITradeTabInterface
    {
        ...
        public void InitializeTab()
        {
            ...
            fast.InitializeProviderControl(model.Fast,false);
        ...
        private void ThreadProc()
        {
            ...
            model.Fast.Bid = 0;
            model.Fast.Ask = 0;
            model.Fast.Time = DateTime.MinValue;
            model.Fast.Volume = 0;
            model.Fast.Balance = null;
            model.Slow.Bid = 0;
            model.Slow.Ask = 0;
            model.Slow.Time = DateTime.MinValue;
            model.Slow.Volume = 0;
            model.Slow.Balance = null;
            model.GapBuy = 0;
            model.GapSell = 0;
            ...
            while (!threadStop.WaitOne(model.SleepMs))
            {
                ...
                if (model.FeederOk)
                {
                    ...
                    for (int i=0;i<connectors.Count;i++)
                    {
                        ...
                        if (i == 0)
                        {
                            model.Fast.Ask = connectors[i].Info.Ask;
                            model.Fast.Bid = connectors[i].Info.Bid;
                            model.Fast.Volume = connectors[i].Info.Volume;
                            model.Fast.Time = connectors[i].Info.Time;
                            model.Fast.Balance = connectors[i].Info.Balance;
                        }
                        if (i == 1)
                        {
                            model.Slow.Ask = connectors[i].Info.Ask;
                            model.Slow.Bid = connectors[i].Info.Bid;
                            model.Slow.Volume = connectors[i].Info.Volume;
                            model.Slow.Time = connectors[i].Info.Time;
                            model.Slow.Balance = connectors[i].Info.Balance;
                        }
                    }
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                Name = model.Fast.Name,
                Symbol = model.Fast.Symbol,
                Prefix = model.Fast.Prefix,
                Postfix = model.Fast.Postfix,
                Parent = model,
