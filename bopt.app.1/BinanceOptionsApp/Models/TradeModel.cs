...
namespace BinanceOptionsApp.Models
{
    ...
    public class TradeModel : BaseModel
    {
        public TradeModel()
        {
            Fast = new ProviderModel();
            Slow = new ProviderModel();
            Slow2 = new ProviderModel();
            Providers = new ObservableCollection<ProviderModel>();
            FastProviders = new ObservableCollection<ProviderModel>();
            SlowProviders = new ObservableCollection<ProviderModel>();
            Fast.Parent = this;
            Slow.Parent = this;
            Slow2.Parent = this;
            Point = 0.00001M;
        ...
        private ProviderModel _Fast;
        public ProviderModel Fast
        {
            get { return _Fast; }
            set { if (_Fast != value) { _Fast = value; OnPropertyChanged(); CreateTitle(); } }
        }
        private ProviderModel _Slow;
        public ProviderModel Slow
        {
            get { return _Slow; }
            set { if (_Slow != value) { _Slow = value; OnPropertyChanged(); CreateTitle(); } }
        }
        ...
        private ObservableCollection<ProviderModel> _FastProviders;
        public ObservableCollection<ProviderModel> FastProviders
        {
            get { return _FastProviders; }
            set { if (_FastProviders != value) { _FastProviders = value; OnPropertyChanged(); } }
        }
        private ObservableCollection<ProviderModel> _SlowProviders;
        public ObservableCollection<ProviderModel> SlowProviders
        {
            get { return _SlowProviders; }
            set { if (_SlowProviders != value) { _SlowProviders = value; OnPropertyChanged(); } }
        }
        ...
        public void CreateTitle()
        {
            if (!Model.Loading)
            {
                ...
                if (Slow != null && Algo != TradeAlgorithm.MultiLegSpread && Algo!= TradeAlgorithm.OneLegMulti)
                {
                    title += " " + Slow.Symbol;
                    var slowProvider = Slow != null ? Model.AllConnections.FirstOrDefault(x => x.Name == Slow.Name) : null;
